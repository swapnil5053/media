...
import type { AccountUsage, Media, MediaSource, MediaStatus } from "@shared/types.js";
...
const SELECT_MEDIA = `
  SELECT m.*, (
    SELECT j.progress FROM jobs j WHERE j.media_id = m.id ORDER BY j.created_at DESC LIMIT 1
  ) AS progress
  FROM media m
`;
...
export function assertWithinQuota(userId: string, plan: PlanId, incomingBytes: number): void {
  ...
  if (incomingBytes > limits.maxUploadBytes) {
    throw payloadTooLarge(`Files on the ${limits.name} plan are limited to ${formatGb(limits.maxUploadBytes)}.`);
  }
  ...
  if (usage.storageBytes + incomingBytes > limits.maxStorageBytes) {
    throw conflict(`This upload would exceed your ${formatGb(limits.maxStorageBytes)} of storage.`, "quota_storage");
  }
}
...
export function markReady(mediaId: string, input: { wasConverted: boolean; hasHls: boolean; hasPoster: boolean }): void {
  ...
  db.prepare(
    `UPDATE media SET status = 'ready', error = NULL, was_converted = ?, has_hls = ?, has_poster = ?,
